...
export default function BlueprintEditModal({
  ...
  const updateBlueprintDimensions = async () => {
    try {
      const response = await api.put(`/api/projects/${project.id}/blueprint`, blueprintForm);
      console.log('Blueprint update response:', response); // Debug log
      
      if (response.success) {
        toast.success('Blueprint dimensions updated successfully');
        
        // Update the city plan data with new blueprint
        const updatedCityPlan = {
          ...cityPlanData,
          blueprint: blueprintForm,
          bounds: {
            minX: -blueprintForm.width / 2,
            maxX: blueprintForm.width / 2,
            minY: -blueprintForm.height / 2,
            maxY: blueprintForm.height / 2
          }
        };
        ...
        toast.error(response.message || 'Failed to update blueprint dimensions');
      }
    } catch (error) {
            ...
            <ModalBody>
              <div className="space-y-4">
                <Input
                  ...
                  onChange={(e) => setBlueprintForm(prev => ({
                    ...prev, 
                ...
                <Select
                  ...
                  onSelectionChange={(keys) => setBlueprintForm(prev => ({
                    ...prev, 
                    unit: Array.from(keys)[0] as 'meters' | 'feet' | 'kilometers'
                  }))}
                >
