...
export class CityPlanUtils {
  ...
  static createDefaultLayers(features: CityFeature[]): {
    [layerId: string]: CityLayer;
  } {
  ...
  private static parseGeometry(geometryInput: string | any): Geometry {
    ...
    // Handle legacy string format like "0,0 10,0 12,1"
    if (typeof geometryInput === "string") {
      const coordPairs = geometryInput.trim().split(/\s+/);
    ...
    console.warn("Unrecognized geometry format:", geometryInput);
    return {
      type: "point",
      coordinates: { x: 0, y: 0 },
    };
